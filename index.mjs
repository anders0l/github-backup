...
function isBackupUpToDate(existingBundles, latestDate) {
	return Object.keys(existingBundles).some((existingBundle) => existingBundle.includes(`${latestDate}`))
}
...
function cleanBundles(bundlePath, existingBundles) {
	if (Object.keys(existingBundles).length + 1 <= parseInt(process.env.MAX_BACKUPS)) return
...
function bundleRepository(repository) {
	const dirname = process.env.BACKUP_PATH || path.dirname(fileURLToPath(import.meta.url))
	const { full_name, clone_url, ssh_url, pushed_at, owner } = repository
	...
	if (isBackupUpToDate(existingBundles, latestDate)) {
		log(chalk.green(`✅  Repository ${full_name} backup is up-to-date. Skipping bundle step.`))
		return
	}
	...
	try {
		...
		const url =
			process.env.USE_SSH_URL === 'true'
				? ssh_url
				: clone_url.replace('https://', `https://${owner.login}:${process.env.GITHUB_TOKEN}@`)
		...
		fs.renameSync(path.join(cloneRepoPath, bundleName), path.join(bundlePath, bundleName))
